...
export function LocationImageManager({
  locationId,
  images,
  onImagesChange,
  disabled = false,
}: LocationImageManagerProps) {
    ...
    async (files: FileList | File[]) => {
      ...
      try {
        for (let i = 0; i < fileArray.length; i++) {
          ...
          // 1. 取得上傳 URL
          const { data: uploadData } = await locationImagesApi.getUploadUrl(
            locationId,
            file.name
          )
          ...
          // 3. 新增圖片記錄到資料庫
          const { data: newImage } = await locationImagesApi.addImage(locationId, {
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    setIsDeleting(true)
    try {
      await locationImagesApi.deleteImage(locationId, deletingImageId)
  ...
  }, [deletingImageId, locationId, onImagesChange])
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        onDrop={handleDrop}
        onClick={() => !disabled && document.getElementById('location-image-upload')?.click()}
      >
        <input
            ...
            {images.map((image, index) => (
              ...
              >
                <img
                  ...
                  alt={image.altText || `據點圖片 ${index + 1}`}
